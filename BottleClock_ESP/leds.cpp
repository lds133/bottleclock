...
void Leds::Animation(int i,int a)
{
    ...
    if (_ledmode[i]==LED_FLASH_STAGE1)
    {
        if (a==0)
        {   digitalWrite(_ledpins[i], _ledhi[i]);
            _ledmode[i]=LED_FLASH_STAGE2;
        } else
        {   digitalWrite(_ledpins[i], _ledlo[i]);
        }
        return;
    ...
    if (_ledmode[i]==LED_FLASH_STAGE2)
    {
        digitalWrite(_ledpins[i], _ledlo[i]);
        return;
    }  
    ...
    {
        if (a % 5==0)
            digitalWrite(_ledpins[i],(a % 10 ==0) ? _ledhi[i] : _ledlo[i]);
        return;
    }  

    if (_ledmode[i]==LED_BLINKHI)
    {
        digitalWrite(_ledpins[i],(a % 2==0) ? _ledhi[i] : _ledlo[i]);
        return;
    }  
